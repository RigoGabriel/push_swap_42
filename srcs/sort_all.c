...
int		find_place_in_a(t_pile *a, int len, int elem, char **rot_type)
{
	...
	if (len == 2 && elem > ft_elem(a, 0) && elem < ft_elem(a, 1))
		...
		place = 0;
	else if (elem > ft_elem(a, find_max_elem(a)) || elem < ft_elem(a, find_min_elem(a)))
		place = find_min_elem(a);
	else
		while (i < len)
		{
			if (elem > ft_elem(a, i) && ((i + 1 < len && elem < ft_elem(a, i + 1)) ||
...
static void	insert_back_in_a(t_pile *a, t_pile *b)
{
	...
	while (pile_count(b))
	{
		num_of_rots = find_place_in_a(a, pile_count(a), ft_elem(b, 0), &rot_type);
		while (num_of_rots > 0)
		{
